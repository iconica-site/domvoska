...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/categories.js


/** @type {HTMLButtonElement} */
const categoriesButton = document.querySelector(".header-button");
/** @type {HTMLElement} */
const categories = document.querySelector(".header-categories");
const max768px = matchMedia("(max-width: 768px)");

if (categoriesButton && categories) {
  categoriesButton?.addEventListener("click", () => {
    categories.classList.add("header-categories--show");
    scrolling.lock();
  });
  ...
  categories.addEventListener("click", event => {
    ...
    if (!target.closest(".header-categories__inner") || target.closest(".header-categories__button")) {
      hideCategories();
    }
  });
  ...
  max768px.addEventListener("change", event => {
    const { matches } = event;

    if (matches) hideCategories();
  });

  function hideCategories() {
    categories.classList.remove("header-categories--show");
    scrolling.unlock();
  }
}
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/stock.js
const articleTitleResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const { target, borderBoxSize } = entry;
    const { blockSize } = borderBoxSize[0];
    const { marginBlockEnd } = getComputedStyle(target);
    const { info } = target.dataset;
    const heightWithMargin = blockSize + parseFloat(marginBlockEnd);

    let sectionInfo;

    try {
      sectionInfo = document.querySelector(`${info}`);
    } catch { }

    sectionInfo?.style.setProperty("--info-padding-block-start", `${heightWithMargin}px`);
  });
});

const articleTitles = document.querySelectorAll("[data-info]");

articleTitles?.forEach(articleTitle => {
  articleTitleResizeObserver.observe(articleTitle);
});
...
const productsSlider = new core(".products-slider", {
  modules: [Navigation, Keyboard,],
  navigation: {
    prevEl: ".products-arrows__button--prev",
    nextEl: ".products-arrows__button--next",
  },
  keyboard: {
    enabled: true,
    onlyInViewport: true,
...
function hideArrowsOnDisabled(swiper) {
  const { el, navigation } = swiper;
  const { prevEl, nextEl } = navigation;
  const arrows = [prevEl, nextEl];
  const isAllArrowsDisabled = arrows.every(arrow => arrow?.classList.contains("swiper-button-disabled"));
  const arrowsBlock = el.querySelector(".products-arrows");

  arrowsBlock?.classList.toggle("products-arrows--hide", isAllArrowsDisabled);
}
...
;// CONCATENATED MODULE: ./src/js/libraries/simplebar/custom-scrolls/choice.js


const choiceTextBlock = document.querySelector(".choice__text");

if (choiceTextBlock) {
  const choiceScroll = new SimpleBar(choiceTextBlock);

  choiceScroll.getScrollElement().addEventListener("scroll", event => {
    const { target } = event;
    const { clientHeight, scrollTop, scrollHeight } = target;
    const isScrollEnd = scrollHeight === clientHeight + scrollTop;

    choiceTextBlock.classList.toggle("choice__text--scroll-end", isScrollEnd);
  });
}

;// CONCATENATED MODULE: ./src/js/libraries/simplebar/simplebar.js
